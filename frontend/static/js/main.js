...
let fullscreenViewer = null; // 全屏查看器
...
function initFullscreenViewer() {
        ...
        <div class="fullscreen-content">
            ...
            <div class="fullscreen-body">
                ...
                <div class="fullscreen-controls">
                    ...
                    <button class="control-btn zoom-reset">
                ...
                <div class="fullscreen-info">
                    <span id="fullscreenInfo">缩放: 100% | 旋转: 0°</span>
                </div>
            </div>
        </div>
    `;
    ...
    style.textContent = `
        ...
        .control-btn.zoom-in { background: #2ecc71; }
        .control-btn.zoom-out { background: #e74c3c; }
        .control-btn.zoom-reset { background: #f39c12; }
        .control-btn.rotate-left { background: #9b59b6; }
        .control-btn.rotate-right { background: #34495e; }
        .control-btn.download-btn { background: #1abc9c; }
    ...
    const zoomInBtn = fullscreenViewer.querySelector('.zoom-in');
    const zoomOutBtn = fullscreenViewer.querySelector('.zoom-out');
    const zoomResetBtn = fullscreenViewer.querySelector('.zoom-reset');
    const rotateLeftBtn = fullscreenViewer.querySelector('.rotate-left');
    const rotateRightBtn = fullscreenViewer.querySelector('.rotate-right');
    const downloadBtn = fullscreenViewer.querySelector('.download-btn');
    ...
    function updateImageTransform() {
        fullscreenImage.style.transform = `translate(${translateX}px, ${translateY}px) scale(${scale}) rotate(${rotation}deg)`;
        fullscreenInfo.textContent = `缩放: ${Math.round(scale * 100)}% | 旋转: ${rotation}°`;
    }
    ...
    if (imagePreview) {
        ...
        imagePreview.addEventListener('click', () => {
            if (imagePreview.src && imagePreview.src !== '') {
                openFullscreenViewer(imagePreview.src);
            }
        });
    }
    
    // 绑定可视化画布点击事件（专门优化可视化图片质量）
    const visualizationCanvas = document.getElementById('visualizationCanvas');
    if (visualizationCanvas) {
        visualizationCanvas.style.cursor = 'zoom-in';
        visualizationCanvas.addEventListener('click', () => {
            if (visualizationCanvas.style.display !== 'none') {
                // 创建更高分辨率的画布用于全屏显示
                ...
                img.onload = function() {
                    // 计算原始图像与可视化画布的缩放比例
                    // 注意：OCR坐标是基于原始图像的，但可视化画布有缩放
                    // 我们需要知道可视化画布相对于原始图像的缩放比例
                    
                    // 获取可视化画布的实际显示尺寸（不是画布像素尺寸）
                    ...
                    // 计算可视化画布的缩放比例（画布像素尺寸 vs 显示尺寸）
                    const canvasPixelWidth = visualizationCanvas.width;
                    const canvasPixelHeight = visualizationCanvas.height;
                    const canvasScaleX = canvasPixelWidth / canvasDisplayWidth;
                    const canvasScaleY = canvasPixelHeight / canvasDisplayHeight;
                    
                    // 计算原始图像到可视化画布的总体缩放比例
                    // 在updateVisualization()中，我们使用了：scaleX = width / img.width
                    // 其中width是画布像素宽度
                    // 所以我们需要反向计算：img.width = width / scaleX
                    // 但我们不知道scaleX，所以需要从画布尺寸推断
                    
                    // 更简单的方法：直接使用OCR返回的原始坐标（基于原始图像）
                    // OCR坐标已经是基于原始图像的，所以我们不需要缩放
                    // 问题在于可视化画布显示时进行了缩放，但全屏查看时我们应该使用原始坐标
    ...
    downloadBtn.addEventListener('click', () => {
        const link = document.createElement('a');
        link.download = `ocr_image_${Date.now()}.png`;
